...
namespace NPOI.OpenXmlFormats.Encryption
{
    public class EncryptionDocument
    {
        ...
        public static EncryptionDocument Parse(XmlDocument xmlDoc) => Parse(xmlDoc, EncryptionNamespaceManager);
        ...
        public static EncryptionDocument Parse(string descriptor)
        {
            ...
            {
                using (var fs = File.OpenRead(descriptor)) xmlDoc.Load(fs);
            }
            else
            {
        ...
        public CT_Encryption GetEncryption() => this.ctEncryption;
        ...
        /// <summary>
        /// Save the encryption descriptor (EncryptionInfo XML) to a stream.
        /// Mirrors Java structure and now serializes password/certificate encryptedKey elements with all attributes.
        /// </summary>
        public void Save(Stream stream)
        {
            if (ctEncryption == null) throw new InvalidOperationException("Encryption data not initialized.");
            ...
            using (var xw = XmlWriter.Create(stream, settings))
            {
                ...
                if (hasPass) xw.WriteAttributeString("xmlns", "p", null, ENCRYPTION_PASSWORD);
                if (hasCert) xw.WriteAttributeString("xmlns", "c", null, ENCRYPTION_CERTIFICATE);

                if (ctEncryption.keyData != null) WriteKeyData(xw, ctEncryption.keyData);
                if (ctEncryption.dataIntegrity != null) WriteDataIntegrity(xw, ctEncryption.dataIntegrity);
        ...
        private static void WriteDataIntegrity(XmlWriter xw, CT_DataIntegrity di)
        {
            ...
            if (di.encryptedHmacKey != null) xw.WriteAttributeString("encryptedHmacKey", Convert.ToBase64String(di.encryptedHmacKey));
            if (di.encryptedHmacValue != null) xw.WriteAttributeString("encryptedHmacValue", Convert.ToBase64String(di.encryptedHmacValue));
            xw.WriteEndElement();
        }
        ...
        private static void WriteKeyEncryptorChild(XmlWriter xw, CT_KeyEncryptor ke)
        {
            ...
            if (child != null)
            {
                ...
                foreach (var name in attrOrder)
                {
                    ...
                    if (!string.IsNullOrEmpty(strVal)) xw.WriteAttributeString(p.Name, strVal);
                }
            }
        ...
        {
            if (!keyEncryptor.uriSpecified) return false;
